...
(use-package general
  ...
  (my/leader-keys
    ...
    "os"  '(org-search-view :which-key "search org files")
    ...
    "ong" '(org-roam-ui-mode :which-key "graph view")
    ...
    "pn"  '(pdf-view-next-page :which-key "next page")
    "pp"  '(pdf-view-previous-page :which-key "prev page")
    "pm"  '(pdf-view-midnight-minor-mode :which-key "dark mode")
    "pk"  '(quit-window :which-key "close pdf")
 
    ;; Rust Development
    "r"   '(:ignore t :which-key "rust")
    "rr"  '(cargo-process-run :which-key "cargo run")
    "rR"  '(my/cargo-run-with-args :which-key "cargo run with args")
...
(use-package eglot-booster
  ...
  :after eglot
  :if (executable-find "emacs-lsp-booster")  ; Only load if binary exists
  :config
  (eglot-booster-mode))
...
;; Custom cargo function with arguments
(defun my/cargo-run-with-args ()
  "Run cargo with custom arguments."
  (interactive)
  (let* ((args (read-string "Cargo run arguments: "))
         (command (if (string-empty-p args)
                      "cargo run"
                    (format "cargo run -- %s" args))))
    (compile command)))
...
(custom-set-faces
 ...
 '(calfw-default-content-face ((t (:foreground "#c678dd" :background unspecified))))
 ...
 '(calfw-periods-face ((t (:foreground "#c678dd" :background unspecified))))
